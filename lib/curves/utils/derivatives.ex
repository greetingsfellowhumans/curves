defmodule Curves.Utils.Derivatives do
  ...
  def apply_derivatives(%Curves.Spline.Curve{segments: segments} = curve, point_derivatives) do
    segments = case point_derivatives do
      [0, 0, 0, 0] -> curve.segments

      _ ->
        Nx.to_list(segments)
          |> Enum.with_index()
          |> Enum.map(&apply_derivatives_to_segment(&1, curve, point_derivatives))
